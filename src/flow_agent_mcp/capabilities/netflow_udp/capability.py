...
from .decoder import decode_netflow


class NetflowUdpCapability:
    """
    NetFlow UDP capability.

    Default port 2055 is common.
    Some exporters use 9995 or other ports, so keep it configurable.

    Decoder stub supports later:
    ...
    async def start(self, host: str, port: int) -> str:
        ...
        self._running = True
        return f"netflow collector started on {host}:{port}"
    ...
    async def _run(self) -> None:
        ...
        try:
            while not self._stop.is_set():
                ...
                flows: List[FlowRecord] = decode_netflow(data)
