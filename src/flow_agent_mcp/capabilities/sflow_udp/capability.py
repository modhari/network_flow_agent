...
from .decoder import decode_sflow


class SflowUdpCapability:
    """
    sFlow UDP capability.

    Collector:
      Receives UDP datagrams on the configured port.

    Decoder:
      decode_sflow transforms raw datagrams into FlowRecord list.

    Note:
      sFlow is binary and may include multiple samples per datagram.
      Latency is not always directly available, so your decode may need to
      compute or attach latency via external correlation.
    """
    ...
    async def start(self, host: str, port: int) -> str:
        ...
        self._running = True
        return f"sflow collector started on {host}:{port}"
    ...
    async def _run(self) -> None:
        ...
        try:
            while not self._stop.is_set():
                ...
                flows: List[FlowRecord] = decode_sflow(data)
