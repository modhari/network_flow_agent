...
from .decoder import decode_ipfix


class IpfixUdpCapability:
    """
    IPFIX UDP capability.

    Default port 4739 is the standard for IPFIX.
    Like NetFlow v9, IPFIX is template based and needs template caching.

    This capability is protocol specific only for ingestion and decoding.
    Everything after decoding is shared across the platform.
    """
    ...
    def register_tools(self, mcp: Any, ctx: CapabilityContext) -> None:
        ...
        @mcp.tool()
        async def start_collection(capability: str, host: str = "0.0.0.0", port: int = 4739) -> str:
    ...
    async def start(self, host: str, port: int) -> str:
        ...
        self._running = True
        return f"ipfix collector started on {host}:{port}"
    ...
    async def _run(self) -> None:
        ...
        try:
            while not self._stop.is_set():
                ...
                flows: List[FlowRecord] = decode_ipfix(data)
